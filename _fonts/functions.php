...
function create_posttype() {
	register_post_type( 'sponsors_supporters',
		array(
			'labels' => array(
	...
	register_post_type( 'home_images',
		array(
			'labels' => array(
				'name' => __( 'Home Images' ),
	...
	register_post_type( 'home_icons',
		array(
			'labels' => array(
				'name' => __( 'Home Icons' ),
				'singular_name' => __( 'Home Icons' ),
			),
	...
	register_post_type( 'staff_and_board',
		array(
			'labels' => array(
...
function add_back_IMG() {  

		if ( wp_is_mobile() ):
			$get_image_size = 'medium';
		else:
			...
			endwhile;
?>
	<div class="content page-top" style="background-image: url(<? echo $image[0] ?>);"></div>
<?
	wp_reset_query();
}  
...
// ********************************** START DISPLAY ATTACHMENTS
	function add_some_box() {
		add_meta_box(
			'display-attachement', // id of the <div> we'll add
			'Icons', //title
			'displayAttachments', // callback function that will echo the box content
			'home_icons', // where to add the box: on "post", "page", or "link" page
			'normal',
			'high'
		);
	//	add_action("admin_print_scripts",'js_libs_attachments');
	//	add_action("admin_print_styles", 'style_libs_attachments');
	...
	if(!empty($images)):
		echo '<style type="text/css">#display-attachement img{ width:400px; margin: 4px; }</style>';
		
		foreach($images as $image):
			$i++;
			$full_image = $image->guid;
			echo '<img src="'. $image->guid.'" border="0">';
			
		endforeach;
	endif;
	echo '<p><a href="#" class="button insert-media add_media" data-editor="content" title="Add Media"><span class="wp-media-buttons-icon"></span> Add ICON</a></p>';
	}
...
// ********************************** END
//include('tweet-php-master/TweetPHP.php');
include('_functions/alldrivers.php');
//include('metrics-dashboard/functions.php');
?>
